  ...
  const initCanvas = (WorkerBase, win) => {
          ...
          this[ContextKey] ||
            (this[ContextKey] =
              (contextType.includes("webgl")
                ? createContextWebGL
                : createContext2D)(this, contextType, contextAttributes));
    ...
    const createContext2D = (
      ...
      const ctx2dGetterMethods =
        "getContextAttributes,getImageData,getLineDash,getTransform,isPointInPath,isPointInStroke,measureText"
          .split(",");
      const CanvasRenderingContext2D = {
        ...
        set(target, propName, value) {
          if ("string" == typeof propName && propName in ctx) {
            ctx[propName] !== value && "function" != typeof value &&
    ...
    const ctxWebGLGetterMethods = "checkFramebufferStatus,makeXRCompatible"
      .split(",");
    const getWebGlMethodCallType = (methodName) =>
      methodName.startsWith("create") || methodName.startsWith("get") ||
      ...
      class extends WorkerEventTargetProxy {
        ...
        static isTypeSupported(mimeType) {
          if (!isStaticTypeSupported.has(mimeType)) {
            const isSupported = callMethod(win, [
              "MediaSource",
              "isTypeSupported",
            ], [mimeType]);
