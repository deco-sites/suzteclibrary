...
((window) => {
  ...
  const isValidMemberName = (memberName) =>
    !(startsWith(memberName, "webkit") || startsWith(memberName, "toJSON") ||
      startsWith(memberName, "constructor") ||
      startsWith(memberName, "toString") || startsWith(memberName, "_"));
    ...
    if (instance) {
      ...
      if (now > lastCleanup + 5e3) {
        instances.forEach((storedInstance, instanceId) => {
          storedInstance[CreatedKey] < lastCleanup && storedInstance.nodeType &&
            !storedInstance.isConnected && instances.delete(instanceId);
        });
        lastCleanup = now;
      }
    }
    ...
    void 0 !== value && (type = typeof value)
      ? "string" === type || "number" === type || "boolean" === type ||
        ...
        ? added.has(value) ? [1, []] : added.add(value) &&
          [1, value.map((v) => serializeForWorker($winId$, v, added))]
        : "object" === type
  ...
  const validCssRuleProps =
    "cssText,selectorText,href,media,namespaceURI,prefix,name,conditionText"
      .split(",");
  const mainAccessHandler = async (worker, accessReq) => {
    ...
    for (; i < totalTasks; i++) {
      try {
        ...
        !winCtxs[winId] && winId.startsWith("f_") &&
        ...
        if (1 === applyPath[0] && applyPath[1] in winCtxs[winId].$window$) {
          ...
          if (instance) {
            ...
            accessRsp.$error$ =
              `Error finding instance "${task.$instanceId$}" on window ${
        ...
        if (!Array.isArray(next)) {
          if ("string" == typeof current || "number" == typeof current) {
            ...
            if ("function" == typeof instance[previous]) {
              ...
              "insertRule" === previous && args[1] > len(instance.cssRules) &&
      ...
      if (scriptElm) {
        ...
        if (!winCtx.$isInitialized$) {
          ...
          ((worker, $winId$, win) => {
            ...
            forwards.map((forwardProps) => {
              ...
              forwardProps.split(".").map((_, i, arr) => {
                mainForwardFn = mainForwardFn[arr[i]] = i + 1 < len(arr)
          ...
          doc.dispatchEvent(new CustomEvent("pt0"));
          {
            ...
            logMain(
              `Executed ${winType} window ${
                normalizedWinId($winId$)
              } environment scripts in ${
                (performance.now() - winCtx.$startTime$).toFixed(1)
              }ms`,
            );
          }
        }
  ...
  const readMainInterfaces = () => {
    const elms = Object.getOwnPropertyNames(mainWindow).map(
      (interfaceName) =>
    ...
    ).filter((elm) => elm).map((elm) => [elm]);
    return readImplementations(elms, []);
  };
  const cstrs = new Set(["Object"]);
  const readImplementations = (impls, interfaces) => {
    const cstrImpls = impls.filter((implData) => implData[0]).map(
      (implData) => {
    ...
    cstrImpls.map(
      ([cstrName, CstrPrototype, impl, intefaceType]) =>
  ...
  const readImplementationMember = (
    ...
    try {
      if (
        isValidMemberName(memberName) && isNaN(memberName[0]) &&
        "all" !== memberName
      ) {
  ...
  const addGlobalConstructorUsingPrototype = (
    ...
    void 0 !== mainWindow[cstrName] &&
      $interfaces$.push([
        cstrName,
        "Object",
        Object.keys(mainWindow[cstrName].prototype).map(
          (propName) => [propName, 6],
        ),
        12,
      ]);
