...
((self) => {
  ...
  const getConstructorName = (obj) => {
    ...
    try {
      const zoneJsConstructorName = null === (_c = null === (_b = null == obj
                    ? void 0
                    : obj.__zone_symbol__originalInstance) || void 0 === _b
              ? void 0
  ...
  const getInstanceCacheKey = (instance, memberName, args) =>
    [
      ...
      memberName,
      ...(args || EMPTY_ARRAY).map(
        (arg) => String(arg && arg[WinIdKey] ? arg[InstanceIdKey] : arg),
      ),
    ].join(".");
  const cachedProps = (Cstr, propNames) =>
    commaSplit(propNames).map(
      (propName) =>
        definePrototypeProperty(Cstr, propName, {
          get() {
  ...
  const serializeForMain = ($winId$, $instanceId$, value, added, type) =>
    void 0 !== value && (type = typeof value)
      ? "string" === type || "boolean" === type || "number" === type ||
          null == value
        ? [0, value]
        : "function" === type
        ? [4, {
        ...
        ? added.has(value) ? [1, []] : added.add(value) && [
          1,
  ...
  const deserializeFromMain = (
    ...
    if (serializedValueTransfer) {
      ...
      if (
        0 === serializedType || 11 === serializedType || 12 === serializedType
      ) {
      ...
      if (1 === serializedType) {
        return serializedValue.map(
          (v) => deserializeFromMain(winId, instanceId, applyPath, v),
        );
      }
  ...
  const getOrCreateSerializedInstance = (
    [winId, instanceId, nodeName, prevInstanceId],
  ) =>
  ...
  const getTargetProp = (target, applyPath) => {
    ...
    if (target) {
      ...
      if ("Window" === cstrName) {
        ...
        n = "nodeType" in target && 2 === target.nodeType
          ? "attributes."
      ...
      target[ApplyPathKey] && target[ApplyPathKey].length &&
  ...
  const getLogValue = (applyPath, v) => {
    ...
    if ("object" === type) {
      ...
      return v[Symbol.iterator]
        ? `[${Array.from(v).map((i) => getLogValue(applyPath, i)).join(", ")}]`
        : "value" in v
        ? "string" == typeof v.value ? `"${v.value}"` : objToString(v.value)
        : objToString(v);
    }
  ...
  const getter = (instance, applyPath, groupedGetters, rtnValue) => {
    ...
    ((
      ...
      if (webWorkerCtx.$config$.logGetters) {
        try {
          const msg = `Get ${getTargetProp(target, applyPath)}, returned: ${
            getLogValue(applyPath, rtnValue)
    ...
    ((target, applyPath, value, restrictedToWorker = false) => {
      if (webWorkerCtx.$config$.logSetters) {
        try {
          ...
          logWorker(
            `Set ${getTargetProp(target, applyPath)}, value: ${
              getLogValue(applyPath, value)
            }${restrictedToWorker ? " (restricted to worker)" : ""}`,
            target[WinIdKey],
          );
        } catch (e) {}
  ...
  const callMethod = (
    ...
    if (
      "setAttribute" === methodName && hasInstanceStateValue(instance, args[0])
    ) {
    ...
    ((target, applyPath, args, rtnValue) => {
      if (webWorkerCtx.$config$.logCalls) {
        try {
          ...
          logWorker(
            `Call ${getTargetProp(target, applyPath)}(${
              args.map((v) => getLogValue(applyPath, v)).join(", ")
  ...
  const constructGlobal = (instance, cstrName, args) => {
    ((target, cstrName, args) => {
      if (webWorkerCtx.$config$.logCalls) {
        try {
          logWorker(
            `Construct new ${cstrName}(${
              args.map((v) => getLogValue([], v)).join(", ")
            })`,
            target[WinIdKey],
          );
        } catch (e) {}
        ...
        constructor(winId, instanceId, applyPath, styles) {
          ...
          return new Proxy(this, {
            get(target, propName) {
              ...
              target[propName] || "string" != typeof propName ||
      ...
      class {
        ...
        insertRule(ruleText, index) {
          ...
          if (index >= 0 && index <= cssRules.length) {
            callMethod(this.ownerNode, ["sheet", "insertRule"], [
              ruleText,
              index,
            ], 2);
  ...
  const runScriptContent = (
    ...
    try {
      webWorkerCtx.$config$.logScriptExecution &&
        logWorker(
          `Execute script: ${
            scriptContent.substring(0, 100).split("\n").map((l) => l.trim())
              .join(" ").trim().substring(0, 60)
          }...`,
          winId,
        );
      ...
      `with(this){${
        scriptContent.replace(/\bthis\b/g, (match, offset, originalStr) =>
          offset > 0 && "$" !== originalStr[offset - 1]
            ...
            : match).replace(/\/\/# so/g, "//Xso")
      }\n;function thi$(t){return t===this}};${
        (webWorkerCtx.$config$.globalFns || []).filter((globalFnName) =>
          /[a-zA-Z_$][0-9a-zA-Z_$]*/.test(globalFnName)
        ).map((g) => `(typeof ${g}=='function'&&(this.${g}=${g}))`).join(";")
      };` + (scriptUrl ? "\n//# sourceURL=" + scriptUrl : "");
    env.$isSameOrigin$ ||
  ...
  const getPartytownScript = () =>
    `<script src="${partytownLibUrl("partytown.js?v=0.8.0")}"><\/script>`;
      ...
      src: {
        ...
        set(url) {
          ...
          if (this.type && config.loadScriptsOnMainThread) {
            const shouldExecuteScriptViaMainThread = config
              .loadScriptsOnMainThread.some((scriptUrl) => scriptUrl === url);
        ...
        insertBefore(newNode, referenceNode) {
          ...
          if (isScript) {
            ...
            if (scriptContent) {
              if (isScriptJsType(scriptType)) {
                ...
                const loadOnMainThread = scriptId &&
                  (null ===
                        (_b = null === (_a = config.loadScriptsOnMainThread) ||
                            void 0 === _a
                          ? void 0
          ...
          if (isIFrame) {
            ...
            ((winId, iframe) => {
              ...
              let callback = () => {
                if (
                  environments[winId] && environments[winId].$isInitialized$ &&
  ...
  const patchHTMLAnchorElement = (WorkerHTMLAnchorElement, env) => {
    ...
    commaSplit("hash,host,hostname,href,origin,pathname,port,protocol,search")
      .map((anchorProp) => {
  ...
  const patchSvgElement = (WorkerSVGGraphicsElement) => {
    const getMatrix = (elm, methodName) => {
      const { a: a, b: b, c: c, d: d, e: e, f: f } = callMethod(elm, [
        methodName,
      ], EMPTY_ARRAY);
  ...
  const createWindow = (
    ...
    const getChildEnvs = () => {
      ...
      for (envWinId in environments) {
        ...
        otherEnv.$parentWinId$ !== $winId$ || otherEnv.$isTopWindow$ ||
    ...
    const WorkerWindow = defineConstructorName(
      class extends WorkerBase {
        constructor() {
          ...
          ((win, nodeCstrs) => {
            ...
            win.customElements = {
              define(tagName, Cstr, opts) {
                ...
                callMethod(win, ["customElements", "define"], [
                  tagName,
                  ceData,
                  opts,
                ]);
              ...
              whenDefined: (tagName) =>
                registry.has(tagName)
                  ? Promise.resolve()
                  : callMethod(win, ["customElements", "whenDefined"], [
                    tagName,
                  ]),
              upgrade: (elm) =>
          ...
          commaSplit(
            ...
            if (!(globalName in win)) {
              ...
              null != value &&
                (win[globalName] = "function" != typeof value ||
            ...
            $window$: new Proxy(win, {
              get: (win, propName) => {
                ...
                if ("string" != typeof propName || isNaN(propName)) {
                  return (null ===
                          (_a = webWorkerCtx.$config$.mainWindowAccessors) ||
                        void 0 === _a
                      ? void 0
        ...
        addEventListener(...args) {
          "load" === args[0]
            ? env.$runWindowLoadEvent$ && setTimeout(() =>
              args[1]({
                type: "load",
              })
            )
        ...
        fetch(input, init) {
          input = "string" == typeof input || input instanceof URL
            ? String(input)
            : input.url;
    ...
    if (webWorkerCtx.$isInitialized$) {
      if (7 === msgType) {
            ...
            try {
              ...
              if (rsp.ok) {
                ...
                let shouldExecute = javascriptContentTypes.some((ct) => {
                  var _a, _b, _c;
                  return null ===
                        (_c = null === (_a = null == responseContentType
                                ? void 0
        ...
        if (1 !== environments[msgValue].$isInitialized$) {
          ...
          logWorker(
            `Initialized ${winType} window ${
              normalizedWinId(winId)
            } environment 🎉`,
            winId,
          );
        }
        ...
        15 === msgType && ((_type, winId, instanceId, callbackName, args) => {
          ...
          elm && "function" == typeof elm[callbackName] &&
